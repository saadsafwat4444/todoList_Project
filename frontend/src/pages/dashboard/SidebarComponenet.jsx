...
//       <div
//         style={{ height: "790px" }}
//         className={`bg-slate-600   text-white w-64 space-y-7 py-12 px-4 absolute md:relative inset-y-0 left-0 transform
//     ${isOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out md:translate-x-0`}
//       >
//         <h1 className="text-2xl font-bold px-4 mb-6 hidden md:block">
//           Dashboard
//         </h1>
...
export function SidebarComponenet() {
  ...
  return (
    <div>
      ...
      <div
        style={{ height: "710px" }}
        className={`bg-slate-600 text-white md:w-64 w-15  space-y-7 py-12 px-4 absolute md:relative inset-y-0 left-0 transform 
        ${isOpen ? "translate-x-0" : "-translate-x-full"} transition-transform duration-300 ease-in-out md:translate-x-0`}
      >
        ...
        <div className="md:hidden flex justify-end mb-4">
          <button
            ...
            className="text-white text-xl p-2 hover:text-red-400"
          >
            <FaTimes />
          </button>
        </div>
        ...
        <nav className="flex flex-col space-y-4">
          ...
          {token && (
            <button
              onClick={logout}
              className="flex items-center space-x-3 px-4 py-3 mt-4 bg-red-600 rounded hover:bg-red-500 transition-colors"
            >
              <FaSignOutAlt />
              <span>Logout</span>
            </button>
